...
| Métrique | Score |
|---|---|
| 📈 **Score Global** | **90.6% ± 0.3%** |
| ✅ Correctness (50% LLM-Judge + 35% Semantic + 15% Keywords) | **83.6%** |
| 🛡️ Faithfulness (fidélité aux sources) | **99.1%** |
| 📚 Source Quality | **99.1%** |
| 🔄 Stabilité inter-runs (σ) | **0.3%** |
...
> \* v8–v9 : scores moyennés sur 3 runs (eval v4 — scoring plus strict, cf. ci-dessous). Non directement comparables aux v1–v7 (eval v3).

#### Changement de scoring (eval v3 → v4)

Les scores v8+ ne sont **pas directement comparables** aux v1–v7 car le système d'évaluation a évolué :

| | Eval v3 (v1–v7) | Eval v4 (v8+) |
|---|---|---|
| Score final | 70% LLM-Judge + 30% Keywords | 55% Correctness + 25% Faithfulness + 20% Sources |
| Correctness | LLM-Judge seul | 50% LLM-Judge + 35% Semantic (BGE-M3 cosine) + 15% Keywords |
| Conciseness | 10% du score | Supprimé (pénalisait les réponses complètes) |
| Multi-run | Non | Oui (moyenne ± σ sur N runs) |
...
│  │ par intent       │     │  ┌────────────────────────────┐  │ │    │
│  │ + Génération LLM │     │  │ enrich → retrieve →        │  │ │    │
│  │                  │     │  │ generate → validate →       │  │ │    │
│  └────────┬─────────┘     │  │ check_completeness →       │  │ │    │
│           │               │  │ respond                    │  │ │    │
│           │               │  └────────────────────────────┘  │ │    │
...
Le pipeline agent utilise un graphe LangGraph avec 7 nœuds et 5 outils locaux (aucun appel externe) :

1. **classify** : Classification d'intent (Phase 0)
2. **enrich** : Décomposition en sous-questions + calculs de dates si nécessaire
3. **retrieve** : Retrieval hybride + reranking
4. **generate** : Génération LLM avec prompt spécialisé par intent
5. **validate** : Vérification des citations (grounding)
6. **check_completeness** : Vérifie que toutes les sous-questions sont couvertes
7. **respond** : Formatage de la réponse finale
